...
test('parseJSON returns error for invalid JSON', () => {
  ...
  assert.ok(typeof r.error.line === 'number');
  assert.ok(typeof r.error.column === 'number');
});
...
test('toYaml handles null and boolean', () => {
  const yaml = toYaml({ a: null, b: true, c: false }).trimStart();
  assert.match(yaml, /a: ~/);
...
test('toYaml quotes strings with special characters', () => {
  const yaml = toYaml({ msg: 'hello: world' }).trimStart();
  // The colon inside the string should cause quoting
  assert.match(yaml, /'/);
});

test('toYaml round-trips via parseJSON', () => {
  const obj = { name: 'Bob', scores: [9, 8, 7], active: true };
...
test('toCsv handles nested keys with dot notation', () => {
  const csv = toCsv([{ user: { name: 'Alice' } }]);
...
test('deep nesting in YAML produces correct indentation', () => {
  const yaml = toYaml({ lvl1: { lvl2: { lvl3: 'deep' } } }).trimStart();
...
test('array of numbers in YAML', () => {
  const yaml = toYaml({ nums: [10, 20, 30] }).trimStart();
