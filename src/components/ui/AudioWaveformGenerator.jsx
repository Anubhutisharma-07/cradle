...
import { Play, Pause, RotateCcw, Upload, Mic, Volume2 } from 'lucide-react';
...
export default function AudioWaveformGenerator() {
  ...
  const drawWaveform = useCallback(() => {
    ...
    const render = () => {
      ...
      ctx.fillStyle = getComputedStyle(canvas).getPropertyValue('--bg-surface') || '#18181b';
  ...
  const togglePlay = async () => {
    ...
    if (!sourceNodeRef.current && audioElementRef.current) {
      try {
        sourceNodeRef.current = audioCtxRef.current.createMediaElementSource(audioElementRef.current);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 sm:p-6 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl text-zinc-100">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h2 className="text-xl font-bold tracking-tight">Audio Waveform Generator</h2>
          <p className="text-sm text-zinc-400 truncate max-w-xs sm:max-w-md">{fileName}</p>
        </div>
